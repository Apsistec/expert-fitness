...
export const stripe = Stripe('pk_test_Rm4QbcP0thjADBses4DnzU5600K3q0XqMA');
...
/*
  ...
  admin.auth().setCustomUserClaims(${UID}, {role: 'admin'}).then(claims =>
    console.log(`CUSTOM CLAIMS ADDED: ${claims}`)).catch(err => console.error(err))
*/
  ...
  if (req.headers?.authorization?.startsWith('Bearer ')){
      ...
      res.status(400).send({'No Bearer Authorization header found': 400});
      next();
  }
}
